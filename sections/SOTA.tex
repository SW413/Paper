...
We deem that it has a high level of abstractions, since it is just function calls, without any memory allocation etc\todo{hvad dækker etc. over}. \citep{Rcuda}                                                 
...
It only natively supports the use of CUDA enabled NVIDIA\todo{Var CUDA kun NVIDIA?} GPUs for its parallel computations on GPUs, but OpenCL extensions do exist such that it becomes possible to use other devices.
...
The code is therefore looking clean without any boilerplate and there is a high level of readability\todo{Vi har ikke skrevet om readability de andre steder. Er det ligemeget?}.
...
They all require the user to specify when to use the GPU instead of the CPU, and the level of abstraction varies, where some require a lot of control of the programmer, and others require specific function calls to GPU functions\todo{Sidste del her er lidt underligt}.
...
The languages do require some knowledge of them, and if one were to choose a language with low level of abstraction, one would need knowledge of allocating memory etc\todo{samme ect som i libraries}.
