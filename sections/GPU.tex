...
\caption{A basic  of the Transistor allocation on the GPU compared to the CPU}\label{image:GPUCPUimage} %ftp://download.nvidia.com/developer/cuda/seminar/TDCI_Arch.pdf
\vspace{-25pt}
\end{figure}
...
\end{figure}
The GPU memory is comprised of the following different memory components, how it is communicates is shown on \todo{OpenCLGPUMemoryModel}. 
...
Although the hierachy and its components were originally designed to improve performance for graphical applications, it can also be used efficiently in some GPU computing applications, how the memory is used is further explained in \myref{sec:opencl}. %http://cuda-programming.blogspot.dk/2013/02/texture-memory-in-cuda-what-is-texture.html
...
For each workgroup, a number of work items exists, these are the threads that execute the kernel.
...
The architectural difference of the GPU from the CPU, are intented to maximise the throughput for problems that can handle a higher latency.
