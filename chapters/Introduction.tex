...
Modern compilers, such as the Intel C++ compiler, can even exploit the \acrshortpl{cpu}  advanced instructions, such as \acrfull{simd} to speed up certain workloads. \citep{INTEL_SIMD}
...
A former engineer from Intel foresees this stagnation of Moore's Law as soon as 2020 or 2022; hence alternatives to using the \acrshort{cpu} architecture needs to be explored.\citep{Moore2013} \todo{Needs er måske lidt for stærkt et ord? -- TK}
...
Even the highest clock speeds, seem to not be improving beyond 9GHz however these clock-speeds produce so much heat, that they require liquid nitrogen for cooling. \todo{Kilde på dette? Er det overhovedet vigtigt? -- TK}
...
\begin{itemize}
	\item How do \acrshortpl{gpu} do parallel calculations, and why and when is it better at it than the \acrshort{cpu}? \todo{Better? Definer lige hvad der menes, fx ``can process more data in the same time'' eller ``more FLOPS'' etc. -- TK}
