...
A former engineer from Intel foresees this stagnation of Moore's Law as soon as 2020 or 2022; hence alternatives to using the \acrshort{cpu} architecture needs to be explored in order to maintain the current growth in speed.\citep{Moore2013} \todo{Needs er måske lidt for stærkt et ord? -- TK}
...
This relatively newfound interest of using the \acrshort{gpu}, is a result of it becoming increasingly difficult to improve upon the \acrshort{cpu}. \todo{Wat? Skal der stå at "There has been relatively?" - Søren | måske bare fjerne relatively? - msm}
...
Even the highest clock speeds, seem to not be improving beyond 9GHz however these clock-speeds produce so much heat, that they require liquid nitrogen for cooling. \todo{Kilde på dette? Er det overhovedet vigtigt? -- TK | sidste to sætninger er ikke relevante - msm}
...
\begin{itemize}
	\item How do \acrshortpl{gpu} do parallel calculations, and why and when is it better at it than the \acrshort{cpu}? \todo{Better? Definer lige hvad der menes, fx ``can process more data in the same time'' eller ``more FLOPS'' etc. -- TK | måske bare skrive ``faster'' - msm}
...
\end{itemize}\todo{Hvad bruger vi disse spørgsmål til? - Corlin}
...
\newpage \todo{Er der en særlig grund til at vi tvinger en ny side her, bryder det ikke bare konsistensen med chap vs section etc? -- TK}
