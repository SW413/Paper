...
A \acrshort{cfg} is a set of formal languages which are useful for specifying syntax. \todo{Det er vist ikke helt rigtigt :P ? - Søren +1 msm}
A \acrshort{cfg} consists of one or more production rules.
On the left side of a production rule is a non-terminal and on the right side are terminals and/or non-terminals, additionally there is a start symbol.\todo{Ambiguous sentence (cfg reference hype!). Meningen er uklar især den sidste del - msm }
...
Having an ambiguous grammar can be a problem when parsing because the one cannot always decide which grammar rule it should apply. \todo{``the one'' hvad? -- Troels | måske bare one ?? - msm}

\subsection{The dangling else problem} \todo{Er der nytte af dette afsnit? -- Troels ... Vi viser vi ved der skal tages højre for det - Corlin}
...
\todo[inline]{lyder ikke godt med alle de A[a-Z ]* - msm}
...
A parser has a lookahead which is the maximum numbers of tokens which are needed to determine what rule should be applied, this is denoted in a parenthesis, so e.g. LL(1) means leftmost derivation using a lookahead of 1 token.\todo{token? måske forklaring - msm}
