...
This function is unique in the sense that it enables the programmer to use strings as a paramater, a data type otherwise not implemented in the language.
...
Another such function is \texttt{MateIntMatrix}.
This creates a matrix with int values a similar function exists for float.
These takes three arguments as input, the amount of rows and colums wished for, aswell as what value to place at every entry.
While this could be done manually for a small matrix, this allows the programmer to initialise a larger matrix and fill it any value in just one function call.
