...
In \gls{gamble} every variable are statically stored, there are several reasons for this and therefore the language does not implement neither explicit or dynamic memory handling.
The first reason being the purpose of the language is calculation operations on the \acrshort{cpu}, we therefore deem this to be a language for smaller applications and thereby the memory is less likely to be filled upon runtime.
A second reason being every variable in \gls{gamble} is immutable.
Memory handing could increase the potentiality for which programs would be possibly in the language and is therefore seen as a viable extension to future versions of \gls{gamble}.
