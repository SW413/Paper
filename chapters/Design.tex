...
``/* */'' is used for multi line comments, meaning that everything between ``/*'' and ``*/'' is ignored by the compiler, also known as C style comments. \todo{Har vi en kilde på disse comment style navne? -Marc}
...
Functions are blocks of code which can be called to do a specific computation, a function can also take some variables as inputs or requite no input.
