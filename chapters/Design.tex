...
The source code must be encoded in UTF-8, but only the ASCII alphabet and numbers are allowed for identifiers and values, this is to simplify the parsing. \todo{Really?? - Søren}
...
Most of the languages examined in \myref{sec:state_of_the_art} used a form of static scope rules. \todo{Det står der da vist ikke noget om :P?  - søren}
...
In \gls{gamble} all variables are statically stored, there are several reasons for this and therefore the language does not implement neither explicit or dynamic memory handling. \todo{Det er ikke dynamic at vi skriver matrix[a,b] hvor a og b er variabler som bestemmes på baggrund af alt muligt ?? - Søren}
...
Any vector and matrix will be allocated on the heap by the compiler. \todo{Compiler igen. - Søren}
Manual memory handling could increase the number of possible programs in the language and is therefore seen as a viable extension to future versions of \gls{gamble}. \todo{Er det relevant ?? - Søren}
...
It is not shown in the table but the $+$ and $-$ operations in \myref{tbl:matOps} requires the matrices and vectors to have the same dimensions.
...
Because of high implementation cost in the compiler, it is not possible to use a function as an input for a function.\todo{Compiler - Søren}
