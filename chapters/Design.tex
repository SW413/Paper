...
\gls{gamble} also have certain functions build into the languange for varying reasons.
The most unique function in question here will be the \texttt{print} function.
This function is unique in the sense that it enables the programmer to use strings, a datatype otherwise not implemented in the languange.
This is enabled such that the programmer can use this feature to write out messages to the console, it is not uncommong to use strings to test if a certain branch of the program is reached which is just one place where this function could be of use.

Other \gls{gamble} provided functions are created out of convenience.
In object-oriented languanges subscripting is quite popular and allows for a wide range of neat features such as \texttt{.GetLength} on arrays.
Similar to this \gls{gamble} provides the \texttt{Rows} and \texttt{Cols} functions for matrix's which returns their amount of rows or columns.
With these functions the programmer does not necessaritly need to remember throughout the program what the dimensions of an arbitrary array is but can use these functions to retrieve that information.
\todo{måske også relevant at nævne Make<T>Matrix func her?}
