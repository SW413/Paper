...
The scope rules of \gls{gabmle} are as follows:
...
This is also easier to parse and write context free grammar for, which also reduces the amount of times the scanner will need to read the source code.
If it was not structured like this, the resulting tree of the code would need to be traversed more than once, to fill the symbol table.
This is done in C\# where a function can be declared later in the document than the function is called.
This results in the value of the function call not being known, and therefore tree must be traversed several times.

We find the feature very good, but we limit ourselves to the more simple structure hence it is a lot easier to implement.
