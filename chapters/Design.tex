...
In order to allow users of \gls{gamble} to use data dynamicly and save the result, there are a built in \texttt{matrixToFile}--function and a \texttt{fileToMatrix}--function. 
...
One of the parameters are a string which identifies the file to be used, outside from the \texttt{print}--function these are the only places where strings are allowed in \gls{gamble}.
