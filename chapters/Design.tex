...
This function is unique in the sense that it enables the programmer to use strings, a data type otherwise not implemented in the language.
This is enabled such that the programmer can use this feature to write out messages to the console. 
This is both useful for output and for debugging. 
%it is not uncommon to use strings to test if a certain branch of the program is reached which is just one place where this function could be of use.
...
Similar to this \gls{gamble} provides \texttt{Rows} and \texttt{Cols} functions for matrix' which returns their amount of rows or columns.
This can be used in loops to make them iterate of matrices of any size.
%With these functions the programmer does not necessarily need to remember throughout the program what the dimensions of an arbitrary array is but can use these functions to retrieve that information.
\improvement[inline]{måske også relevant at nævne Make<T>Matrix func her? Enig - Troels}
