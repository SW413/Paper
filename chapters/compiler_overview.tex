...
When creating a compiler there are two basic ways of compilers one can create, a multi-pass compiler or a single-pass compiler.\todo{..ways of compilers..}
...
The choice of language for writing the compiler falls upon Java 1.8, which is the language used in the language and compiler course.
...
This is the scanner's job, the parser structures these tokens into a tree structure, which can be traversed in the path the source code is written.
...
The last phase of the compiler is code generation and optimising.
...
This separation helps give an overview when working with the compiler, all the phases are distinctly separated, which makes it easier to make changes, and locate where in the compiler changes need to be made.
