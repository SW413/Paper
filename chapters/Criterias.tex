...
\todo{måske Robert W. Sebesta, ved første benævnelse? dunno} Sebesta have given some criterias which can be evaluated from, and we will take these criterias into account when designing our language.\citep{Sebesta}
...
The language supports abstraction over the programmer's\todo{måske platform? -  det kan køres på flere end programmørens maskine.} hardware, by not having to specify where the computations must be run, or having to allocate memory.
...
Our overall thoughts of our language according to Sebesta's criterias can be found in \myref{sec:phil}.\todo{msåke noget ala, "a more precise description of our language will be presented and argued for in the following chapter"?}
