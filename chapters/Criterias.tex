...
It is so because of the goals stated in \myref{sec:problem} and based on the other languages examined in \myref{sec:state_of_the_art}, simplicity is important when designing a language where the focus lies in the writing code fast and easy, that can take advantage of the \acrshort{gpu}.
It is so because this goal does not require a large language.
Based on the earlier discussions, exception handling is seen as a was of making the language more complex, therefore a goal in \gls{gamble} is to catch errors like type mismatch on compile time, which strongly eliminates the need for exception handling.
Similar arguments have been used on all the language concepts in \myref{tbl:concepts} to evaluate each concepts in respect the wanted features of \gls{gamble}. 
