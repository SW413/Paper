...
Every big-step semantic showing operations on a vector can not be used on a matrix, since it requires the dimensions to fit those of a one-dimension vector.\todo{Det her er vist ikke helt rigtigt? Snak om det på gruppen}
...
\subsection*{Declaring Variables}\TODO{Skal skrives om - er det samme som fra bogen}
...
The transition relation describing variable declarations defines a big-step semantic, since the allocation of new address space for newly declared variables is an invisible operation.\todo{Det her forstår jeg altså ikke helt ? Hvad betyder det overhovedet?}
...
Type in the variable declaration can be a int, float or bool.
...
Declaring a matrix of a given type is generated with ${ v }_{ 1 }$ number of rows and ${ v }_{ 2 }$ number of columns. \todo{Hvorfor * ??}
...
\begin{aligned}
	\frac { { e }_{ v },st\vdash { M }^{ 1 }{ \rightarrow  }_{ A }{ v }_{ 1 }\quad { e }_{ v },st\vdash { M }^{ 2 }{ \rightarrow  }_{ A }{ v }_{ 2 } }{ { e }_{ v },st\vdash { M }^{ 1 }*{ M }^{ 2 }{ \rightarrow  }_{ A }{ v } } ,\begin{matrix} { v }_{ 1 }=matrix<>[j,k] \\ { v }_{ 2 }=matrix<>[k,l] \\ v=\begin{bmatrix} { { m }_{ 1,1 }^{ 3 } } & { { m }_{ 1,1 }^{ 3 } } & \dots  & { m }_{ 1,l }^{ 3 } \\ { { m }_{ 2,1 }^{ 3 } } & +{ { m }_{ 2,2 }^{ 3 } } & \dots  & { { m }_{ 2,l }^{ 3 } } \\ \vdots  & \vdots  & \ddots  & \vdots  \\ { m }_{ j,1 }^{ 3 } & { { m }_{ j,2 }^{ 3 } } & \dots  & { m }_{ j,l }^{ 3 } \end{bmatrix}where \\  \end{matrix}
\end{aligned}
\end{equation}
\begin{equation*}
\begin{aligned}
...
\end{equation*}
\end{minipage}
\todo{Skal der være punktummer ligesom i i den ovenfor for at vise vi går ned på en ny række måske?}
...
\begin{equation}
	\frac { { e }_{ v },st\vdash { M }^{ 1 }{ \rightarrow  }_{ A }{ v }_{ 1 }\quad { e }_{ v },st\vdash { a }_{ 1 }{ \rightarrow  }_{ A }{ v }_{ 2 } }{ { e }_{ v },st\vdash { M }^{ 1 }\ast { a }_{ 1 }{ \rightarrow_{ A }}{ v } } ,\begin{matrix} { v }_{ 1 }=matrix<>[j,k] \\ { v }_{ 2 }={ a }_{ 1 } \\ v=\begin{bmatrix} { { a }_{ 1 }*m }_{ 1,1 }^{ 1 } & { { a }_{ 1 }*m }_{ 1,2 }^{ 1 } & \dots  & { { a }_{ 1 }*m }_{ 1,k }^{ 1 } \\
...
\todo{Skal man ikke skrive at a =>a v ? Altså at man kan evaluere expression til en value?}