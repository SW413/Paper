...
Declaring a matrix, a matrix of a given available type is generated with the size of $v_{1}*v_{2}$.
...
Matrix product also called matrix multiplication takes two matrices of same type, where the size of $m^{1}$ and $m^{2}$satisfy $m^{1}[j,k]*m^{2}[k,l]$.
...
A scalar product is a simple operation on a matrix,where every element in the $j,k$ matrix is multiplayed with a Arithmetic expression $a$. 
