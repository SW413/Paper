...
As a result of how the \acrshort{gpu} is used in the current version of \gls{gamble}, the operations and functions created by the programmer, may very well use the \acrshort{gpu} but not as effectively as it could in all cases.\todo{er ikke helt med her - Søren}
...
As aforementioned \gls{gamble} performs all operations using matrix and vectors on the \acrshort{gpu}.\todo{Men vi havde ikke divider hver index:P ? - Søren}
...
A possible expansion would be to increase runtime efficiency further to only utilise the \acrshort{gpu} when an increase in performance is to be expected from doing so. \todo{Hvis størrelsen af en matrice kan være dynamisk, så kan STATISK analyse ikke hjælpe dig med at vælge imellem gpu og cpu? -- Troels}
...
For further development of \gls{gamble} one should also consider using the \acrshort{gpu} for more than matrix and vector operations and hereby developing the language to do \gls{gpgpu}. \todo{Står det ikke på linien lige overfor o.o ?}
Rather than blindly disregarding loops one could have the compiler check whether or not the loop in question could be parallelised, and perhaps perform some of the increase its runtime efficiency as mentioned in \myref{subsec:runtime} upon said loop.
...
With a deeper understanding of how the different memory sections communicate, it is possible to increase performance by using the best suited part of the memory hierarchy. \citep{ocl_lecture3} \todo[inline]{Fandt en kilde som siger at det kan koste 400-600 cycles for at tilgå global memory :O \url{http://on-demand.gputechconf.com/gtc-express/2011/presentations/NVIDIA_GPU_Computing_Webinars_Best_Practises_For_OpenCL_Programming.pdf} -- Troels}
...
An example would be that while AMD \acrshort{gpu}s have hardware to support both integer and float calculations, NVIDIA \acrshort{gpu}s only has for floats. \citep{CUDAOpenCLOptimisation} \todo[inline]{Er dette stadig korekt? Måske bare at de ikke er optimerede til det? Dette er godt nok CUDA docs men der står stadig integer operationer: \url{http://docs.nvidia.com/cuda/cuda-c-programming-guide/} del 5.4.1 -- Troels}
...
\todo{ Er det så teksnisk set ikke også dækket af lineær algebra :P ? - Søren}
...
Alternatively one could be content with adding new data types and operations for other fields and implement the field specific methods as libraries.\todo{Data types ? really? - Søren}
...
At this point \gls{gamble} is rather narrowly focused on certain areas like matrix and vector operations, the language could utilise its strengths to expand focus to relevant similar domains.\todo{Dette er hvad ovenstående afsnit nævner så kan godt ske det skal omformuleres en smule - Marc}
In the following paragraphs some of the key aspects to expand the language are discussed. \todo{Er denne sætning nødvendig? -- Troels}
...
A full implementation of strings would also be beneficial for a more general purpose language.\todo{delete this line? MP - General purpose, meget fy fy! - Marc}
...
Making \gls{gamble} object oriented with common features like inheritance and polymorphism would allow many programmers to write programs similar to what they are used to.\todo{der skal virkelig nogle argumenter her ind omkring hvorfor det ville hjælpe et sprog rettet mod linear algebra - Marc}
This would make it easier to write large programs. \todo{Men er det den type programmer du vil skrive i Gamble? -- Troels}
...
These libraries could therefore use similar ways of using the \acrshort{gpu} as matrices already do.\todo{Men det går meget væk fra matematik tho, og er allerede et meget veletableret område når det kommer til at bruge GPU'en - det ligger sku endda i navnet - Marc}
