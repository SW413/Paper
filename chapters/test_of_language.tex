\chapter{Test of Language Runtime Efficiency} % (fold)
\label{cha:test_of_language}
In order to evaluate the run-time efficiency of \gls{gamble} a test is performed. 
...
Previously C code for the \acrshort{cpu} has been compared to CUDA C code, both code examples written by the project group, running on the \acrshort{gpu}, see \myref{sub:gpubenchmark}.
