...
The source code must be encoded in UTF-8, but only the ASCII alphabet and numbers are allowed for identifiers and values, this is to simplify the parsing. \todo{Really?? - Søren, and how does that simplify parsing - Marc? Regex: \[A-Za-z\]\[A-Za-z0-9\_\] -- TK | DFA'en er ikke lige så kompliceret som hvis karaktersættet var større = simplere parsing - msm}
The language is case sensitive, because the most used languages seen on TIOBEs index are also case sensitive. \citep{TIOBE}
When a language is case sensitive it is possible for the programmer to name different parts of the code like he pleases, resulting in higher readability. \todo{Måske noget med at kunne bruge standarder indenfor programmering med lokale variabler er med små bogstaver osv? | Man skal bare passe på men konventioner indbygget i syntaxen - msm}
...
The use of newline is often used in simple languages e.g. MATLAB or other mathematical oriented languages.\todo{ Simple languages? Hvordan er det defineret - msm}
...
As mentioned before forgotten semicolons can result in compile errors, we deem this as the biggest disadvantage of this end of statement terminator.\todo{det er vel egentligt parsing errors... - msm}
...
If a programming language contains no implementation\todo{``implementation'' ? -- TK} of scope rules, one must assume that all the source code in a given program would be in the same scope. 
