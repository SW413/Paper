...
\acrshort{antlr} is based upon a parser technique called LL(*).\todo{vi har allerede nævnt antlr en del før her. MP - Dog ikke i denne detalje, ville foreslå forklaringen kommer op til første sted hvor det er nævnt - Marc Tidligere stod antlr til at være ALL(*) -- Troels}
LL(*) uses an algorithm to have a varying lookahead when needed.
The LL(*) parsers, which is a parser that upholds an LL(*) grammar, allows a bigger class of \acrshort{cfg}s than other parsers like LL(k). \todo{This is wrong?! Det er samme class af gramatiker, men kan performe bedre da ``k'' er dynamisk. -- Troels}
...
\todo[inline]{Et lidt uklart afnsit, virker til en del ting bliver fortalt flere gange på forskellige måder. MP Enig, det hele virker meget redundant. -- Troels}
...
expr : expr '*' expr 	#MulExpression // match expressions with * operator
     ...
     ;
\end{lstlisting}\todo{Hvorfor er denne ambiguous ? Den bruger left recursion, men den er da ikke ambigous er den ? - Søren - Fordi hvis ikke for precedence reglen som ANTLR bruger har 2+2*2 2 parse trees - Marc}
