...
This as a result means that to achieve the utmost benefit from each individual platform, an implementation for each platform should be available; having to alter the implementation of kernels to each platform makes the cross platform feature less useful than initially thought.\todo{utmost virker lidt woldsomt, no ? - Søren}
...
\myref{image:OpenCLOptCompare} shows a significant performance difference between the native implementations of CUDA and OpenCL is shown.\todo{Passer ikke helt med den figur der bliver vist, jeg ser i vært faldt ikke noget explicit OpenCL og Cuda sammenligning - Corlin.}
...
This would have made using CUDA more beneficial in terms of performance, as attempting to do so in OpenCL C through compiler optimisations requires an intricate knowledge of compiler creation and \acrshort{gpu} computing.\todo{Det her afsnit fattede jeg honestly totalt hat af :p - Søren}
...
This could possibly have led to greater performance from \acrshort{gamble} at the cost of only being of use to users with NVIDIA hardware.\todo{Måske noget om hvor meget af markede de sidder på for at få det til at lyde som en bedre ide - Søren}
