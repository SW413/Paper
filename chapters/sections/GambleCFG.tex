...
The first production from the \acrshort{cfg} that will be the statement. Looking at \myref{lst:statements} it is seen that a statement can be both a assignment, declaration, functioncall, controlblock and a loop. Statement productions is the building blocks of the code.
...
\begin{lstlisting}[caption={\acrshort{cfg} Expression},frame=tlrb,label={lst:declaration},numbers=none]
expression
...
In this production are there both termials, and non-terminals.
