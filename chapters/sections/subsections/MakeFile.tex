...
After the \gls{gamble} compiler have compiled the source code; the compiler outputs OpenCL C code as its object code.\todo{Er det ikke ligegyldigt efterhånden ? We know this - Søren}
...
Additionally the \gls{gamble} standard library, containing functions such as \texttt{matrixToFile} and \texttt{fileToMatrix} must also be included and compiled. \todo{Troede det var en del af grammatikken ?? No ? - Søren}
...
The same executable file is not usable for another platform, e.g. ARM, nor on another operating system, e.g. Windows or OS X.\todo{Skal vi ændre dette til OS X only med gamble? - Wath ? Og desuden så har vi ikke skrevet hverken i konklusionen eller noget at det kun virker på linux og osx har vi ?}
