...
It is however possible to implicitly convert between integer and floating point types internally e.g. from int16 to int64, as long as the destination variable is of a larger bit-width. \todo{Er dette imprementeret? -- Troels}
...
For very operator it must be checked whether its operands' types match. \todo{``very''-operator, that's a new one, what does it do ;) -- Troels}
...
A type mismatch error is caused by a value or identifier not being compatible (type safe) with a function's parameter, an operator etc. \todo{Dette får det til at lyde som om det kun gælder for funktioner? - Søren .. Der står ``an operator''? -- Troels}
...
\gls{gamble} validates that the size of matrices and vectors match for specific \gls{gamble} operations as addition and multiplication etc. as well as if they are out of bounds. \todo{Boundstjek bliver indsat ved code gen, ikke her? -- Troels}
...
The method visits an \texttt{ExpressionNode} and calls \texttt{CombineValueTypes} with the nodes' left and right values, if there are any available, otherwise it sends \texttt{null} as the input value. \todo{Marc her er din kommentar til koden vel også? :) -- Troels}
...
After the type checker have checked every expression\todo{Det er vel ikke kun expressions som tjekkes? eller er alt et subset af expressions? -- Troels} in the source code, the compiler scans for unused variables and afterwards prints all these as warnings.
