...
The scope checker produces two errors: redeclaration errors and undeclared errors defined in \myref{subsec:Scope}.\todo{reference nødvendig igen? also: can produce. MP}
...
If no entry is found, the search continues in the \texttt{symbolMap} of the enclosing scope and so on recursively.\todo{Må man ikke redeclare et variable i et indre scope ??, så hvis jeg bruger tmp ude i global scope, så må jeg ikke bruge den inde i en funktion ?? - Søren}
