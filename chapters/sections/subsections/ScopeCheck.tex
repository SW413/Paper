...
The scope checker can produce two types of errors: redeclaration errors and undeclared errors defined in \myref{subsec:Scope}.\todo{reference nødvendig igen? also: can produce. MP}
A redeclaration error is produced when an attempt to declare a variable while it is already declared in scope is made.\todo{klumset formulering. MP.. Forslag:``A redeclaration error is an attempt to declare a variable with a name already in use.'' -- Troels}
...
/* [...] */
\end{lstlisting}
\todo[inline]{Evt lav en UML diagram der viser de klasser der bruges til at lave symbol tabellet, det gør i hvert fald det nemmere at forstå implementation. Men et diagram hører til i design afsnittet.}
...
\todo{Meget forvirrende at holde styr på uden en figur tbh. - Søren  .. En listing som viste klassen eller et diagram kunne være brugbart. -- Troels}
...
If no entry is found, the search continues in the \texttt{symbolMap} of the enclosing scope and so on recursively.\todo[inline]{Må man ikke redeclare et variable i et indre scope ??, så hvis jeg bruger tmp ude i global scope, så må jeg ikke bruge den inde i en funktion ?? - Søren - Ikke ifølge hvad der står her - Marc .. Du må gerne bruge det samme variabel navn i en funktion som i global scope, men du må ikke redeclare det i et indre, fx et loop. Derudover så vil scope checking forgå for hver funktion før global scopes body. (linje 11). Men en omformulering kunne nok godt være på plads. -- Troels )}
