...
\end{lstlisting}
\todo[inline]{maybe refactor 'filesNstuff'? MP}
...
\texttt{global\_size} and \texttt{local\_size} are there to determine the amount of memory needed both globally and locally on the \acrshort{gpu}.\todo{Disse er hardcoded til et index på  2 i listingen, eventuel forklar hvorfor? - Marc}
...
When an assignment's right side or a declaration's right side consists of an expression using operators and matrices or vectors, the visitor checks which operator is used and then inputs a launch kernel template depending on the operator.
...
\end{lstlisting}\todo{kunne det eventuel være smart at lave en liste over hvad de arguementer betyder? altså R, r, w, a osv. ? - Marc}
