...
In fact whenever one of the following operators \texttt{+, -, *, \#, \^{} } are used with matrices or vectors a template is being appended to the outputCode. \todo{Ikke alle disse er i tabellen i language design? -- Troels #DetErDeNu! :D - Søren}
...
\end{lstlisting}\todo[inline]{kunne det eventuel være smart at lave en liste over hvad de arguementer betyder? altså R, r, w, a osv. ? - Marc | Måske bare henvise til SimpleCL specifications - msm | En henvisning ville nok være rigelig tror jeg ? Hvad synes i ? - Søren}
...
If the size of a matrix was 4\todo{betyder dette en 2x2 matrice eller en 4x4 matrice? - Corlin}, a kernel would be launched with 1, 2, 3 and 4 as the indices. \todo{Er de ikke nul indekserede? -- Troels}
...
If the matrix form is needed the rows of the matrix would be places in \texttt{global\_size[0]} and the columns in \texttt{global\_size[1]}.\todo{Er i tvivl om hvad der skal stå her ? Places in ? - Søren}
