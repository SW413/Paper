...
\acrshort{antlr} generates a parser which produces a parse tree that contains information about how the parser have grouped the tokens into more abstract language definitions, such as expressions and statements.\todo{Måske sætte en reference ind" More about Antlr in ref til impl om antlr ?" - Søren"} \todo{Vi bruger 2 linjer på at vælge en parser generator, andre bruger flere sider? skal vi have mere om pro-cons? -- Troels}
...
Recursive-descent parsers are a collection of recursive methods, one per rule of the \acrshort{cfg}. \todo{Dette er en gentagelse af linje 14? -- Troels}
