...
As the \acrshort{gpu} is the resource being used to achieve more computational power, the data computed must also be applicable to the niche of the \acrshort{gpu}, i.e. the data must be parallelisable as explained in \myref{sec:comparch}.
...
\info[inline]{Maybe we should add automatic memory management here, it is *HARD* in \gls{opencl}. \gls{cuda} etc. so it would make a nice point? -- Troels}
...
This is because of the large overhead as explained in \myref{sec:comparch}, it will be faster to perform a test of an algorithm with smaller datasets in other languages.
