...
This is done by using a C-like syntax, while stripping \gls{gamble} of features we deem not necessary for a language which main focus lies in using the \acrshort{gpu} for  linear algebra calculations. \todo{``striping'' antyder at vi starter med C og fjerner stuff. -- TK}
The C-like syntax is chosen because the top 5 languages on Tiobe's list of most popular programming languages are C-like, and therefore if \gls{gamble} resembles these languages, it will be more familiar to start programming in \gls{gamble} which in turn makes it easier to implement ones algorithms. \citep{TIOBE}\todo{Lidt kringlet sætning med 2x gamble - Corlin}
...
\todo{Denne udtalelse gør at vi nok bør lytte til Thomas og helt slette afsnit 14.6 om general purpose Gamble.. - Søren, Enig -- TK :)}
...
This is because of the large overhead as explained in \myref{sec:comparch}, it will be faster to perform a test of an algorithm with smaller datasets in other languages, like C.\todo{Claimer vi at C er et sprog man hurtigt implemterer algoritmer i? passer dette? MP}
Therefore to use the niche that \gls{gamble} proclaims, having the language be familiar makes it easier to use for its pure computational aspect, and improves on \gls{gamble}'s read- and writability.\todo{Blive read- and writability højre bare fordi der er sprog der ligner det? - Hvad hvis der er folk som ikke har siddet i C-like, så bliver det ikke højre - Corlin}
