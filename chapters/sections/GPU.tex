...
The \acrshort{cpu} consists of only a few cores which are very fast at performing sequential serial processing. \citep{whatisgpu} \todo{Sequential og Serial, betyder disse ikke det samme i denne kontekst? -- TK}
In recent years developers and scientists seem to have developed an interest for performing calculations on Dedicated Graphic Processing Units. \citep{gpurise}\todo{Dette er eneste gang vi benytter dette udtryk, ændres til GPU? MP} \todo{``seem to have'' virker meget fluffy, skal det omformuleres? -- TK}
This is a different architecture which was intended for graphics processing.\todo{Hvad referere -This is a different architecture- til? - Corlin}
...
The screen can then be divided into different sections called blocks and the computations for each block of data is divided internally on the \acrshort{gpu} to its threads. \todo{Har en GPU tråde? Eller mener vi at tråde, eller subproblemer, fordeles over GPUens beregningsenheder? -- TK}
...
Unlike the cores in the \acrshort{cpu}; the cores in the \acrshort{gpu} are not designed to perform sequential serial processing, instead its cores are designed to handle multiple tasks at once. \todo{Igen sequential serial processing? -- TK}
...
Due to their architectural differences the \acrshort{gpu} allows for about 12 times more operations per second.\todo{Vi kan sætte en fodnote ind med de beregninger der var her tidligere evt :P ? - Søren ... Tallene er 384 GFLOPS for i7'en og 4612 GFLOPS for 980'eren -- TK}

This makes the \acrshort{gpu} particularly useful for large and complex computations if it is\todo{when it is, måske MP .. Enig -- TK} possible to distribute the workload among the \acrshort{gpu}'s cores.
However, \todo{indsæt evt. using her.} the \acrshort{gpu} has an overhead cost. 
...
To illustrate this problem as well as finding an approximate computation size where the \acrshort{gpu} is advantageous some test problems have been written. \todo{problem have been written? Måske en omformulering vil være passende? -- TK}
...
This is a large amount of data but this is partly due to the simple operations done on the elements in the matrix, we believe that the threshold would happen earlier for more complex operations on the elements in matrix.\todo[inline]{og hvorfra ved vi at det er fordi det er simple operationer? og hvorfor mener vi thresholden ændres med andre operationer? -- ?? .. Det er fordi at her er overførslen af data meget stor i forhold til beregningen O(n**2) og O(n**2), hvor matrix mul er O(n**2) og O(n**3), altså vokser antallet af beregninger mere end antallet af overførsler og derfor bruges en større del af kørsen på beregninger end overførsel, og der er GPUen god. -- TK}
In addition to the threshold, the result also confirms that as the data set grows the \acrshort{gpu} becomes exponentially faster at computing the data. \todo{Er dette forhold exponentiel eller liniært? -- TK}
